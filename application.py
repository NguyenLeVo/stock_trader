...
## API_KEY=pk_40e6fcd0466446f7981fc1994a3102ac
...
def add_funds():
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash + :amount WHERE id =:user_id", user_id=session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Check if shares were positive integer
        try:
        ...
        # How much $$$ the user still has in the account
...
def quote():
    ...
    # User reaced route via GET (as by clicking a link or via redirection)
    else:
...
def register():
    ...
    if request.method == "POST":
        ...
        # Hash the password and isert a new user in the database
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = db.execute("SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = :user_id AND symbol = :symbol GROUP BY symbol", user_id=session["user_id"], symbol=request.form.get("symbol"))
